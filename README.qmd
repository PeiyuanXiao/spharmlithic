...
A pre-built Docker image includes R, RStudio Server, and the full
Python environment. On macOS the direction-vector pipeline works in a
native installation, but `spharm_from_meshes()` does not: the OpenMP
runtime bundled with `open3d` clashes with the one that comes with R,
which aborts the R session with `OMP: Error #15`. Use Docker there.
...
Open <http://localhost:8787> in your browser (user: `rstudio`,
password: `rstudio`). Example data and a quick-start script are
pre-loaded in `~/examples/`.
...
For detailed usage, see the two vignettes: `vignette("Introduction", package = "spharmlithic")` (from artefacts to spherical harmonic coefficients) and `vignette("Statistics", package = "spharmlithic")` (PCA, PERMANOVA, and tests of standardisation on the results).

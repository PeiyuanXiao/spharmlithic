...
#' Install the Python backend for `lithicscarpattern`
#'
#' Creates (or recreates) a conda environment containing the Python packages
#' required for spherical harmonic analysis. By default installs the **core**
#' environment, sufficient for the direction-vector pipeline
#' ([spharm_from_directions()]). Pass `mesh = TRUE` to additionally install
#' `trimesh` and `open3d` for the STL pipeline ([spharm_from_meshes()]).
#'
...
#' @param new_env Logical. If `TRUE` (default when `envname` is the package
#'   default), remove any existing environment with the same name first.
...
#' }
#'
#' @seealso [use_lithicscarpattern_python()]
#' @export
install_lithicscarpattern_python <- function(
    envname         = "r-lithicscarpattern",
    method          = c("auto", "conda", "virtualenv"),
    mesh            = FALSE,
    python_version  = ">=3.9,<3.13",
    new_env         = identical(envname, "r-lithicscarpattern"),
  ...
  message("\nlithicscarpattern: Python backend installed in environment '",
          envname, "'.\n",
          "  Restart R, then call library(lithicscarpattern) to use it.")
...
#' Point `lithicscarpattern` at an existing Python environment
#'
#' Use this if you already have a conda environment with the required Python
#' packages (e.g. an environment created by your analysis project's
...
#' or [spharm_from_meshes()] call, and ideally before the package's first use
#' in an R session.
#'
...
#' @param check_mesh Logical. If `TRUE`, also verify mesh-extension packages
#'   (`trimesh`, `open3d`). Default `FALSE`.
#'
#' @return Invisibly, a list with elements `envname`, `python` (path to
#'   the active Python executable), and `available` (named logical vector
#'   of required-package availability).
#'
#' @examples
#' \dontrun{
#' # Reuse the conda env created by your analysis project
#' use_lithicscarpattern_python("spharm")
#'
#' # Reuse and verify mesh extension is available
#' use_lithicscarpattern_python("spharm", check_mesh = TRUE)
#' }
#'
#' @seealso [install_lithicscarpattern_python()]
#' @export
use_lithicscarpattern_python <- function(envname, check_mesh = FALSE) {
  ...
  if (length(missing)) {
    warning("The following Python packages are NOT available in '", envname,
            ...
            "\n  Install them, or run install_lithicscarpattern_python(",
            if (check_mesh) "mesh = TRUE" else "",
            ").",
            call. = FALSE)
  } else {
    message("lithicscarpattern: using Python from '", envname,
